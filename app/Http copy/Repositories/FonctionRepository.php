...
class FonctionRepository
{
    ...
    public function getAll($request)
    {
        ...
        $req = Fonction::ignoreRequest(['per_page'])
    ...
    {
        $model = new Fonction($data);
        $aws= new AwsService();
        if(request()->file('file'))  $model->file = $aws->upload(request()->file('file'),"Fonctions")['full_url'];
        $model->save();
    ...
    {
        $model = Fonction::findOrFail($id);
        $aws= new AwsService();
        if(request()->file('file'))  $data['file'] = $aws->upload(request()->file('file'),"Fonctions")['full_url'];
        $model->update($data);
    ...
    function generateLink($id,$data) {
        $code = Core::generateUniqueCode(Fonction::class, 10, 'FET');
        $link_token = Str::random(40); // Génère un token de 40 caractères
        $url = env('APP_FRONT_URL').'/Fonction/'.$code.'/'.$link_token;
    ...
    function generateMediaLink($id) {
        ...
        $url = env('APP_FRONT_URL').'/Fonction-gallery/'.$code.'/'.$media_token;
        $url = mb_convert_encoding($url, 'UTF-8', 'auto'); // Force l'encodage en UTF-8
    ...
    function verifyLink($data) {
        if (isset($data['link_token'])) {
            return Fonction::where('link_token', )->first();
        }else{
            return Fonction::where('media_token', $data['media_token'])->first();
    ...
    function participate($data){
        $check=Invite::where('Fonction_id', $data['Fonction_id'])
