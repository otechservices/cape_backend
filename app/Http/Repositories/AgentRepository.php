...
class AgentRepository
{
    ...
    function generateLink($id,$data) {
        $code = Core::generateUniqueCode(Agent::class, 10, 'FET');
        $link_token = Str::random(40); // Génère un token de 40 caractères
        $url = env('APP_FRONT_URL').'/Agent/'.$code.'/'.$link_token;
    ...
    function generateMediaLink($id) {
        ...
        $url = env('APP_FRONT_URL').'/Agent-gallery/'.$code.'/'.$media_token;
        $url = mb_convert_encoding($url, 'UTF-8', 'auto'); // Force l'encodage en UTF-8
    ...
    function verifyLink($data) {
        if (isset($data['link_token'])) {
            return Agent::where('link_token', )->first();
        }else{
            return Agent::where('media_token', $data['media_token'])->first();
    ...
    function participate($data){
        $check=Invite::where('Agent_id', $data['Agent_id'])
