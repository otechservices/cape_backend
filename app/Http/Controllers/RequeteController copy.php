...
class RequeteController extends Controller
{
    ...
    static public function store($request)
    {
       ...
       if($service==null){
        ...
        if($request->hasFile('files')){
            ...
            try {
                $unzipper  = new Unzip();
                $filenames = $unzipper->extract($request->file('files'),public_path('docs/'.$code));
                File::makeDirectory(public_path('docs/'.$code),$mode = 0777, true, true);
          
                $fileInputs=$request->fileInputs;
                $i=0;
                //var_dump(json_decode($fileInputs)[0]->name);
                foreach ($filenames as $value) {
                    //$filename= FileStorage::setFile("doc_store",$value,$code,time());
                    RequeteFile::create([
                        "type"=>"PDF",
                        "reference"=>json_decode($fileInputs)[$i]->name,
                        "filename"=>$value,
                        "level"=>0,
                        "file_id"=>json_decode($fileInputs)[$i]->file_id,
                        "requete_id"=>$requete->id,
                    ]);
                    $i++;
                }
    ...
    public function getForSession($code)
    {
        ...
        if ($requete->session->is_active ===null) {
            $requetes=Requete::with(['files.file', 'files.myAvis','session','TypeCape','service','district.Municipality.Department'])->where('code',$code)->first();
        }else
        if ($requete->session->is_active ==true) {
         
            $requetes=Requete::with(['files.file', 'files.myAvis','files.avis.sm.member','files.avis.requeteFile.file','session','TypeCape','service','district.Municipality.Department','referals'])->where('code',$code)->first();
        }elseif ($requete->session->is_active==false) {

            $requetes=Requete::with(['files.file', 'files.myAvis','files.avis.sm.member','session','referals','TypeCape','service','district.Municipality.Department'])->where('code',$code)->first();
    ...
    public function finishStore2(Request $request)
    {
        ...
        return response()->json([
            "success"=>true,
            "message"=>"Dépôt physique enregistré",
            "data"=>[
               "file" =>$recFile
            ]
        ], 200);
